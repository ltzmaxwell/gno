...
// dosDepth on THIS branch is the demo setting. 24 is ~8.5s per walk, enough to see
// clearly while still finishing. Raise it to watch the node stop making progress
// instead: 30 is ~60s, 40 is ~2^41 node visits — hours. There is no pass condition
// for that, which is the point. A halt cannot be asserted, only observed.
const dosDepth = 24
...
// DEMONSTRATION BRANCH. This file is checked out on origin/master (#6088 landed),
// where the go/types validType walk is unpriced, so the assertions below FAIL BY
// SUCCEEDING — slowly. That is the demonstration: read the t.Logf lines, not the
// pass/fail. The same file passes in ~2s on the branch that prices the walk.
//
// Run it:
//
//	cd contribs/gpao && go test ./... -run TestDoSGapAgainstARealChain -v -timeout 30m
//
// Expected here (Apple M5):
//
//	submit               91ms   parks, no type-check    <- #6088 working as designed
//	gpao simulate        8.9s   verdictReady            <- the oracle would APPROVE
//	enable               9.2s   SUCCEEDED, 3.2M gas     <- 9s of CPU, not in the bill
//	stranger MsgRun      9.8s   SUCCEEDED, 1.6M gas     <- no oracle, no policy
//	30 composed MsgRun   10.0s  client timeout          <- still running at the deadline
//
